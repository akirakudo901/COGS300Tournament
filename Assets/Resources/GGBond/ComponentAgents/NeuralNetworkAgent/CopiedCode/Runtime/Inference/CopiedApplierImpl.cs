...
namespace CopiedCode
{
    ...
    /// </summary>
    internal class CopiedContinuousActionOutputApplier : CopiedTensorApplier.IApplier
    {
        readonly CopiedActionSpec m_ActionSpec;

        public CopiedContinuousActionOutputApplier(CopiedActionSpec actionSpec)
        {
            m_ActionSpec = actionSpec;
        }

        public void Apply(CopiedTensorProxy tensorProxy, IList<int> actionIds, Dictionary<int, ActionBuffers> lastActions)
        {
            ...
            for (var i = 0; i < actionIds.Count; i++)
            {
                ...
                if (lastActions.ContainsKey(agentId))
                {
                    ...
                    {
                        actionBuffer = new ActionBuffers(CopiedActionSpec.ConvertToActionSpec(m_ActionSpec));
    ...
    /// </summary>
    internal class CopiedDiscreteActionOutputApplier : CopiedTensorApplier.IApplier
    {
        readonly CopiedActionSpec m_ActionSpec;


        public CopiedDiscreteActionOutputApplier(CopiedActionSpec actionSpec, int seed, ITensorAllocator allocator)
        {
            m_ActionSpec = actionSpec;
        }
    ...
    /// </summary>
    internal class CopiedLegacyDiscreteActionOutputApplier : CopiedTensorApplier.IApplier
    {
        readonly int[] m_ActionSize;
        readonly CopiedMultinomial m_Multinomial;
        readonly CopiedActionSpec m_ActionSpec;
        readonly int[] m_StartActionIndices;
        readonly float[] m_CdfBuffer;


        public CopiedLegacyDiscreteActionOutputApplier(CopiedActionSpec actionSpec, int seed, ITensorAllocator allocator)
        {
            m_ActionSize = actionSpec.BranchSizes;
            m_Multinomial = new CopiedMultinomial(seed);
            m_ActionSpec = actionSpec;
            m_StartActionIndices = CopiedUtilities.CumSum(m_ActionSize);
        ...
        internal void ComputeCdf(CopiedTensorProxy logProbs, int batch, int channelOffset, int branchSize)
        {
    ...
    /// </summary>
    internal class CopiedMemoryOutputApplier : CopiedTensorApplier.IApplier
    {
        Dictionary<int, List<float>> m_Memories;

        public CopiedMemoryOutputApplier(
