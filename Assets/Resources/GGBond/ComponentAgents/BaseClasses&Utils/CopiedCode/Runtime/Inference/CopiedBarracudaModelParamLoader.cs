...
namespace CopiedCode 
{
    ...
    /// </summary>
    public class CopiedBarracudaModelParamLoader
    {
        ...
        public static FailedCheck CheckModelVersion(Model model)
        {
            ...
            var memorySize = (int)model.GetTensorByName(CopiedTensorNames.MemorySize)[0];
            ...
            )
        {
            ...
            if (memorySize == -1)
            {
                failedModelChecks.Add(FailedCheck.Warning($"Missing node in the model provided : {CopiedTensorNames.MemorySize}"
                    ));
                return failedModelChecks;
            }
        ...
        )
        {
            ...
            if ((brainParameters.VectorObservationSize != 0) &&
                (!tensorsNames.Contains(CopiedTensorNames.VectorObservationPlaceholder)))
            {
            ...
            for (var sensorIndex = 0; sensorIndex < sensors.Length; sensorIndex++)
            {
                ...
                if (sensor.GetObservationSpec().Shape.Length == 3)
                {
                    if (!tensorsNames.Contains(
                        CopiedTensorNames.GetVisualObservationName(visObsIndex)))
                    {
                        failedModelChecks.Add(
                ...
                {
                    if (!tensorsNames.Contains(
                        CopiedTensorNames.GetObservationName(sensorIndex)))
                    {
                        failedModelChecks.Add(
            ...
            {
                if (!tensorsNames.Contains(CopiedTensorNames.ActionMaskPlaceholder))
                {
                    failedModelChecks.Add(
        ...
        )
        {
            ...
            if (memory > 0)
            {
                ...
                if (!tensorsNames.Any(x => x == CopiedTensorNames.RecurrentInPlaceholder))
                {
                    failedModelChecks.Add(
                            FailedCheck.Warning("The model does not contain a Recurrent Input Node but has memory_size.")
                            );
        ...
        static IEnumerable<FailedCheck> CheckOutputTensorPresence(Model model, int memory)
        {
            ...
            if (memory > 0)
            {
                ...
                if (!allOutputs.Any(x => x == CopiedTensorNames.RecurrentOutput))
                {
                    failedModelChecks.Add(
        ...
        static FailedCheck CheckVisualObsShape(
            CopiedTensorProxy tensorProxy, ISensor sensor)
        {
            ...
            int observableAttributeTotalSize)
        {
            var failedModelChecks = new List<FailedCheck>();
            var tensorTester =
                new Dictionary<string, Func<BrainParameters, CopiedTensorProxy, ISensor[], int, FailedCheck>>()
            {
                {CopiedTensorNames.VectorObservationPlaceholder, CheckVectorObsShapeLegacy},
                {CopiedTensorNames.PreviousActionPlaceholder, CheckPreviousActionShape},
                {CopiedTensorNames.RandomNormalEpsilonPlaceholder, ((bp, tensor, scs, i) => null)},
                {CopiedTensorNames.ActionMaskPlaceholder, ((bp, tensor, scs, i) => null)},
                {CopiedTensorNames.SequenceLengthPlaceholder, ((bp, tensor, scs, i) => null)},
                {CopiedTensorNames.RecurrentInPlaceholder, ((bp, tensor, scs, i) => null)},
            };
            ...
            for (var sensorIndex = 0; sensorIndex < sensors.Length; sensorIndex++)
            {
                ...
                if (sens.GetObservationSpec().Shape.Length == 3)
                {
                    ...
                    tensorTester[CopiedTensorNames.GetVisualObservationName(visObsIndex)] =
                        (bp, tensor, scs, i) => CheckVisualObsShape(tensor, sens);
                    visObsIndex++;
                }
                if (sens.GetObservationSpec().Shape.Length == 2)
                {
                    tensorTester[CopiedTensorNames.GetObservationName(sensorIndex)] =
        ...
        static FailedCheck CheckVectorObsShapeLegacy(
            BrainParameters brainParameters, CopiedTensorProxy tensorProxy, ISensor[] sensors,
            int observableAttributeTotalSize)
        {
        ...
        static FailedCheck CheckPreviousActionShape(
            BrainParameters brainParameters, CopiedTensorProxy tensorProxy,
            ...
            ActuatorComponent[] actuatorComponents)
        {
            ...
            {
                var modelDiscreteBranches = model.GetTensorByName(CopiedTensorNames.DiscreteActionOutputShape);
